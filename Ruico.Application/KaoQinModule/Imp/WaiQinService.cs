...
{
    public class WaiQinService : IWaiQinService
    {
        IWaiQinRepository _Repository;

        #region Constructors

        public WaiQinService(IWaiQinRepository repository)                               
        {
            if (repository == null)
        ...
        *************/
        private void OperationLog(string action, WaiQinDTO itemDto, WaiQinDTO oldDto = null, UserDTO operatorDTO = null)
        {
            OperateRecorder.RecordOperation(itemDto.Id.ToString(), action,
                itemDto.GetOperationLog(oldDto), operatorDTO);
        }

        public WaiQinDTO Add(WaiQinDTO itemDto, UserDTO operatorDTO = null)
        {
            ...
            this.OperationLog(KaoQinMessagesResources.Add_WaiQin, model.ToDto(),  null, operatorDTO);
        ...
        public void Approve(WaiQinDTO itemDto, UserDTO operatorDTO = null)
        {
            //get persisted item
        ...
        private void UpdateDepartmentOpinion(WaiQin persistedModel, WaiQinDTO itemDto, UserDTO operatorDTO = null)
        {
        ...
        private void UpdateGeneralManagerOfficeOpinion(WaiQin persistedModel, WaiQinDTO itemDto, UserDTO operatorDTO = null)
        {
            var oldDTO = persistedModel.ToDto();
        ...
        public void Remove(Guid id)
        {
            ...
            this.OperationLog(KaoQinMessagesResources.Remove_WaiQin, memberDto, null);

            //commit unit of work
            _Repository.UnitOfWork.Commit();
        }

        public WaiQinDTO FindBy(Guid id)
        {
            var persistedModel = _Repository.Get(id);
        ...
        public IPagedList<WaiQinDTO> FindBy(KaoQinConditionDTO condition, int pageNumber, int pageSize)
        {
            var list = _Repository.FindBy(condition.ToModel(), pageNumber, pageSize);

            var result = list.ToList();

            return new StaticPagedList<WaiQinDTO>(
                result.Select(x => x.ToDto()),
                pageNumber,
                pageSize,
                list.TotalItemCount);
        }
        public DataTable GetExportTable(List<WaiQinDTO> items)
        {
            var result = new DataTable("外出申请记录");
            result.Columns.Add("员工编号");
            result.Columns.Add("姓名");
            result.Columns.Add("部门");
            result.Columns.Add("职位");
            result.Columns.Add("外出时间");
            result.Columns.Add("返回时间");
            result.Columns.Add("外出地点");
            result.Columns.Add("外出事由");
