...
{
    public class ChuChaiRepository : EfRepository<ChuChai>, IChuChaiRepository
    {
        public ChuChaiRepository(IUnitOfWork unitOfWork)
            : base(unitOfWork)
        {
        }

        public IPagedList<ChuChai> FindBy(KaoQinCondition condition, int pageNumber, int pageSize)
        {
            IQueryable<ChuChai> entities = Table;
            ...
            return new StaticPagedList<ChuChai>(
                result,
                pageNumber,
                pageSize,
                totalCount);
        ...
        public bool Exists(ChuChai item)
        {
            IQueryable<ChuChai> entities = Table;
